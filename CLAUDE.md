...
**Settings / Remote Config editor:** `Settings` has two tabs — General (profile, admin key, workspace info) and System configuration, a Firebase Remote Config template editor (`src/lib/remoteConfig.ts`). Privileged Remote Config access lives in the Vercel serverless function `api/remote-config.ts` (GET → `{ etag, template }`, PUT → validate/publish): it mints a Google access token from a service-account key held ONLY in the `RC_SERVICE_ACCOUNT` env var (full JSON, never `VITE_*` — this key can rewrite production wallet addresses), and gates every request by verifying the caller's Firebase ID token and checking the email against the `RC_ADMIN_EMAILS` (comma-separated) allowlist — fails closed if either var is unset. The browser client sends the signed-in admin's ID token; no OAuth popup, no client-side secret. (A previous direct-from-browser OAuth approach died on Google's consent screen: neither `firebase.remoteconfig` nor unregistered sensitive scopes are grantable → `invalid_scope`.) Publish uses `If-Match: <etag>` optimistic concurrency (409 on conflict) plus a confirm modal, since publishing goes live to the mobile app. JSON parameter values are pretty-printed for editing and minified back on publish; only `defaultValue` is editable (conditional values are surfaced but untouched). Local dev: `vite dev` has no serverless functions — set `VITE_RC_PROXY_BASE` in `.env.local` to the deployed dashboard origin (the function sends CORS headers), or run `vercel dev`.
